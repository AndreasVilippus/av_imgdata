...
def test_checks_changed_since_days_input_is_visible_but_only_enabled_for_scan():
    ...
    assert 'v-if="vm.selectedChecksAction === \'scan\'"' not in block
    assert "v-model.number=\"vm.checksChangedSinceDays\"" in block
    assert ":disabled=\"vm.checksLoading || vm.selectedChecksAction !== 'scan'\"" in block
