...
TEST = Path("tests/test_checks_findings_stop_action.py")
...
def main() -> None:
    if not TEST.exists():
        raise SystemExit(f"Missing file: {TEST}")
    TEST.write_text(NEW_TEST, encoding="utf-8")
    print("Updated checks findings stop action test to match current optimizations mixin.")
    print("Next:")
    print("  PYTHONPATH=src python3 -m pytest tests/test_checks_findings_stop_action.py")
