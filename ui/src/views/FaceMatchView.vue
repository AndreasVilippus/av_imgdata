					...
					<div class="face-match-status-card face-match-status-card-action">
						<div class="face-match-status-head">
							<div class="sm-section-title">{{ vm.$avt('face_match:card_action', 'Action') }}</div>
								<div v-if="vm.faceMatchLoading || (vm.faceMatchRecognitionActionSelected && vm.faceMatchRecognitionCleanupActive)" class="face-match-status-running">
									<span class="sm-loader"></span>
									{{ vm.$avt('face_match:card_running', 'Running') }}
								</div>
							</div>
							<div v-if="vm.faceMatchRecognitionActionSelected && vm.faceMatchRecognitionStatusMessage && (!vm.isCleanupProgressForAction('recognition_analyze_unknown_faces') || Number(vm.getCleanupStatusProgress().total) <= 0)" class="face-match-status-message">{{ vm.faceMatchRecognitionStatusMessage }}</div>
							<div v-else-if="!vm.faceMatchRecognitionActionSelected && !vm.faceMatchShowStoredFindingsProgress && !vm.faceMatchShowPersonsProgress && !vm.faceMatchShowFileProgress" class="face-match-status-message">{{ vm.faceMatchStatusMessage }}</div>
