<template>
	<section class="panel">
		...
		<div class="config-actions config-actions-right">
			<v-button @click="loadAll" :disabled="loading || saving || enrolling" style="width: 160px;">
				{{ $avt('config:button_reload', 'Reload') }}
			</v-button>
			<v-button @click="saveConfig" :disabled="loading || saving || enrolling" style="width: 160px;">
		...
		<div v-if="loading" class="config-loading">
			...
			{{ $avt('external_worker:loading', 'Loading external worker settings...') }}
		</div>
		...
		<div v-else class="config-layout">
			<section class="config-card">
				...
				<div class="config-form-grid">
					<label class="config-checkbox">
						<input v-model="workerApi.ENABLED" type="checkbox" :disabled="saving" />
			...
			<section class="config-card">
				...
				<div class="config-form-grid external-worker-registration-form">
					<label class="config-field">
						<span class="config-field-label">{{ $avt('external_worker:worker_name', 'Worker name') }}</span>
						<input v-model="enrollmentForm.enrollment_id" type="text" class="config-text-input" :disabled="enrolling" placeholder="windows-worker-01" />
					</label>
					<label class="config-field">
						<span class="config-field-label">{{ $avt('external_worker:expires_minutes', 'Code validity in minutes') }}</span>
						<input v-model.number="enrollmentForm.expires_minutes" type="number" min="1" max="1440" class="config-text-input" :disabled="enrolling" />
					</label>
				</div>

				<div class="config-actions">
					<v-button @click="startEnrollment" :disabled="enrolling || !enrollmentForm.enrollment_id.trim()" style="width: 190px;">
						{{ enrolling ? $avt('external_worker:registering', 'Starting...') : $avt('external_worker:start_registration', 'Register worker') }}
					</v-button>
				</div>
				...
				<div v-if="activeEnrollment.enrollment_code" class="external-worker-code-box">
					<div>
						...
						<div class="config-card-desc">{{ $avt('external_worker:registration_expires', 'Expires: {expires}', { expires: activeEnrollment.expires_at || '-' }) }}</div>
					</div>
				...
				<div v-if="enrollmentRows.length" class="external-worker-table-list">
					<div v-for="item in enrollmentRows" :key="item.enrollment_id + ':' + item.created_at" class="external-worker-list-row">
						<div>
							<div class="external-worker-bundle-title">{{ item.enrollment_id }}</div>
							<div class="config-card-desc">{{ item.worker_id || $avt('external_worker:not_connected', 'Not connected yet') }}</div>
						</div>
			...
			<section class="config-card">
				...
				<div v-else class="external-worker-table-list">
					<div v-for="worker in workerRows" :key="worker.worker_id" class="external-worker-list-row">
						<div>
							<div class="external-worker-bundle-title">{{ worker.worker_id }}</div>
							<div class="config-card-desc">{{ worker.version || 'unknown' }} · {{ worker.capabilities.join(', ') || '-' }}</div>
							<div class="config-card-desc">{{ $avt('external_worker:last_seen', 'Last seen: {time}', { time: worker.last_seen_at || '-' }) }}</div>
						</div>
			...
			<section class="config-card">
				...
				<div v-else class="external-worker-bundle-list">
					<div v-for="bundle in bundleRows" :key="bundle.target" class="external-worker-bundle-row">
						<div>
							<div class="external-worker-bundle-title">{{ bundle.target }}</div>
							<div class="config-card-desc">{{ bundle.bundle_path }}</div>
							<div class="config-card-desc">{{ $avt('external_worker:binary_status', 'Binary: {path} ({status})', { path: bundle.binary_relative_path || '-', status: binaryStatusLabel(bundle) }) }}</div>
						</div>
...
export default {
	...
	computed: {
		...
		archiveFormatsText() {
			const formats = Array.isArray(this.packageStatus.archive_formats_checked) ? this.packageStatus.archive_formats_checked : [];
	...
	methods: {
		...
		async callApi(apiPath, body = {}) {
			const resp = await fetch(apiPath, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json', 'X-SYNO-TOKEN': this.getSynoToken() }, body: JSON.stringify({ ...body, cookies: this.collectDsmCookies(), synoToken: this.getSynoToken() }) });
		...
		normalizeWorkerApi(input) {
			const workerApi = (input && typeof input === 'object' && !Array.isArray(input)) ? input : {};
			return { ENABLED: Boolean(workerApi.ENABLED), STATE_PATH: String(workerApi.STATE_PATH || '').trim() };
		},
		normalizePackageStatus(input) {
			const status = (input && typeof input === 'object' && !Array.isArray(input)) ? input : {};
			return { ...this.createDefaultPackageStatus(), ...status, archive_formats_checked: Array.isArray(status.archive_formats_checked) ? status.archive_formats_checked : [], bundles: Array.isArray(status.bundles) ? status.bundles : [], download_ready: Boolean(status.download_ready), build_process_requires_archives: Boolean(status.build_process_requires_archives) };
		},
		statusLabel(bundle) {
			if (bundle.download_ready) return this.$avt('external_worker:status_archive_ready', 'Archive ready');
			if (bundle.bundle_exists) return this.$avt('external_worker:status_archive_missing', 'Archive missing');
			return this.$avt('external_worker:status_bundle_missing', 'Bundle missing');
		},
		binaryStatusLabel(bundle) {
			if (!bundle.binary_exists) return this.$avt('external_worker:status_missing', 'missing');
			if (bundle.binary_location === 'archive') return this.$avt('external_worker:status_present_in_archive', 'present in archive');
			return this.$avt('external_worker:status_present', 'present');
		},
		enrollmentStatusLabel(status) {
			if (status === 'enrolled') return this.$avt('external_worker:enrolled', 'Enrolled');
			if (status === 'expired') return this.$avt('external_worker:expired', 'Expired');
			return this.$avt('external_worker:waiting', 'Waiting');
		},
		async loadAll() {
			this.loading = true;
			this.message = '';
			try {
				const configResponse = await this.callApi('/webman/3rdparty/AV_ImgData/index.cgi/api/config_get');
				const config = (configResponse && configResponse.data && configResponse.data.config) || {};
				this.configModel = { ...config, worker_api: this.normalizeWorkerApi(config.worker_api) };
				this.workerApi = this.normalizeWorkerApi(this.configModel.worker_api);
				await Promise.all([this.loadStatusOnly(), this.loadEnrollmentStatus()]);
		...
		async saveConfig() {
			...
			this.message = '';
			try {
				const payloadConfig = { ...this.configModel, worker_api: this.normalizeWorkerApi(this.workerApi) };
				const data = await this.callApi('/webman/3rdparty/AV_ImgData/index.cgi/api/config_save', { config: payloadConfig });
				const config = (data && data.data && data.data.config) || payloadConfig;
				this.configModel = { ...config, worker_api: this.normalizeWorkerApi(config.worker_api) };
				this.workerApi = this.normalizeWorkerApi(this.configModel.worker_api);
				this.message = this.$avt('external_worker:message_saved', 'External worker settings saved. Restart the package if the Worker API activation changed.');
				await this.loadStatusOnly();
			} catch (err) { this.message = `Error: ${err.message}`; } finally { this.saving = false; }
		},
		async startEnrollment() {
			this.enrolling = true;
			this.message = '';
			try {
				const response = await this.callApi('/webman/3rdparty/AV_ImgData/index.cgi/api/external_worker_enrollment_start', this.enrollmentForm);
				this.activeEnrollment = (response.data && response.data.enrollment) || {};
				this.enrollmentStatus = (response.data && response.data.status) || { enrollments: [], workers: [] };
				this.message = this.$avt('external_worker:registration_started', 'Registration started. Copy the one-time code to the worker.');
			} catch (err) { this.message = `Error: ${err.message}`; } finally { this.enrolling = false; }
		},
		async copyEnrollmentCode() {
			try {
				await navigator.clipboard.writeText(String(this.activeEnrollment.enrollment_code || ''));
				this.message = this.$avt('external_worker:code_copied', 'Registration code copied.');
			} catch (err) { this.message = `Error: ${err.message}`; }
		},
		async loadEnrollmentStatus() {
			const response = await this.callApi('/webman/3rdparty/AV_ImgData/index.cgi/api/external_worker_enrollment_status');
			this.enrollmentStatus = (response && response.data) || { enrollments: [], workers: [] };
		},
		async loadStatusOnly() {
			const statusResponse = await this.callApi('/webman/3rdparty/AV_ImgData/index.cgi/api/external_worker_status');
			this.packageStatus = this.normalizePackageStatus(statusResponse && statusResponse.data && statusResponse.data.package);
		},
	},
};
</script>
...
.external-worker-bundle-actions { align-items: flex-end; display: flex; flex-direction: column; gap: 8px; }
